...
def validate_server_id_format(server_id: Union[str, int, None]) -> bool:
    ...
    # Convert to string
    str_id = standardize_server_id(server_id)
    if str_id is None:
        return False
...
async def get_server(db, server_id: Union[str, int, None], guild_id: Union[str, int, None]) -> Optional[Dict[str, Any]]:
    ...
    """
    # Standardize IDs for consistent handling
    str_server_id = standardize_server_id(server_id)
    str_guild_id = standardize_server_id(guild_id)
...
async def check_server_existence(
    ...
    if is_guild_object_pattern is not None:
        ...
        db = guild_or_db
        str_guild_id = standardize_server_id(guild_id_or_db)
...
def legacy_standardize_server_id(server_id: Union[str, int, None]) -> str:
    ...
    """
    return standardize_server_id(server_id) or ""
...
async def validate_server_access(db, server_id: Union[str, int, None], guild_id: Union[str, int, None], user_id: Union[str, int, None] = None, required_feature: str = None) -> Tuple[bool, Optional[str]]:
    ...
    try:
        str_server_id = standardize_server_id(server_id)
        str_guild_id = standardize_server_id(guild_id)
        str_user_id = standardize_server_id(user_id) if user_id is not None else None
    except Exception as e:
        ...
        try:
            ...
            if str_user_id is not None:
                try:
                    ...
                    standardized_admin_uids = [standardize_server_id(uid) for uid in admin_users if uid is not None]
                    ...
                    if premium_tier >= 2:
                        ...
                        standardized_allowed_uids = [standardize_server_id(uid) for uid in allowed_users if uid is not None]
...
async def check_server_limits(db, guild_id: Union[str, int, None]) -> Tuple[bool, int, int]:
    ...
    try:
        ...
        for server in servers:
            ...
            server["server_id"] = standardize_server_id(server_id)
            if server["server_id"]:
